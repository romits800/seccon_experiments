...
case $aarch in
    mips)
          suffix=mips; target=mips; arch=mips; cpu=mips32; flags="-mfloat-abi=soft";;
    thumb)
          suffix=cm0; target=armv6; arch=thumb; cpu=cortex-m0; flags="-mthumb -mfloat-abi=soft";;
    *)
          echo "Give architecture as the third argument"; exit 0;;
esac

clang-3.8 -c -nostdlib -ffreestanding  --target=$target   -mcpu=$cpu $flags -o ${filename}_$suffix.ll  -emit-llvm $filename.cpp
