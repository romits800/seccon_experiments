...
$LLC -march=thumb -mcpu=cortex-m0  -regalloc=basic -debug-only=regalloc $FILE.ll -o ${FILE}_cm0.s  &> /dev/null
# generates shared_variable.txt
${FSE_HOME}/common/analy_backend_linux  memBool.log shared_variable.txt

# generates the z3 file
$OPT -march=thumb -mcpu=cortex-m0 -load ${FSE_HOME}/LLVM/build/lib/LLVMfactInt.so -factInt -input $INPUT -share shared_variable.txt $FILE.ll
...
# generate fixed file
$LLC -march=thumb -mcpu=cortex-m0 -regalloc=basic  -debug-only=isel $FILE.ll -o ${FILE}_cm0_new.s &> /dev/null
